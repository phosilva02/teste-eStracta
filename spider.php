...
class WebCrawler {
    ...
    //Função principal do Spider. Irá receber o cnpj digitado, pegar a página e retornar os dados. Tambem irá verificar se a página
    //recuperada indica algum erro e lancar uma excecao.
    public function search($cnpj) {
        ...
        if(preg_match('/NúMERO\sDE\sCONTROLE/is', utf8_encode($html))) {
            throw new \Exception("CAPTCHA INVALIDO");
        } 
        
        if(preg_match('/CNPJ\sN\S{1,8}O\sCADASTRADO/is', utf8_encode($html))) {
            throw new \Exception("CNPJ NAO CADASTRADO");
        }
        if(preg_match('/INSCRI\S{1,16}O\sCNPJ\sINV\S{1,8}LIDA/is', utf8_encode($html))) {
    ...
    private function quebrarCaptcha(){
        ...
        //cUrl passando a URL onde será feita a requisição
        $ch = curl_init('http://www.sintegra.fazenda.pr.gov.br/sintegra/captcha?'. ($numero_aleatorio*2));
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_COOKIEFILE, 'cookies.txt');
        curl_setopt($ch, CURLOPT_COOKIEJAR, "cookies.txt");
    ...
    private function parserHTML($html){
        ...
        do {
            ...
            if(count($outro_ie[0]) == 0){
                break;
            }
            $contador = $contador + 1;
            preg_match_all('/value="([^"]+)"/is', $html, $valorAnterior);
            //print_r($valorAnterior[1][1]);
            //Dados da requisição post
            $postData = array(
                '_method'=> 'POST',
                'data[Sintegra1][campoAnterior]' => $valorAnterior[1][1],
                'consultar'=> ''
            );
            $header = array(
                ...
                'Connection'=>'keep-alive',
                'Referer'=>'http://www.sintegra.fazenda.pr.gov.br/sintegra/sintegra1',
                //'Cookie'=>'CAKEPHP=2497abe4ad68a3223e7960d33376868d',
                'Upgrade-Insecure-Requests'=>'1'
            );
            ...
            //curl_setopt($ch, CURLOPT_VERBOSE, true);
            $html = curl_exec($ch);
            file_put_contents('saida3.html', $html);
            curl_close($ch);
        } while (count($outro_ie[0]) != 0);
